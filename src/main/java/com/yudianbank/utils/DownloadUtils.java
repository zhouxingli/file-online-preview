...
//        type = dealWithMS2013(type);
    ...
    /**
     * 对最有一个路径进行转码 对带有fileName的进行判断
     * http://192.168.100.33:8300/reportTemplateExport/ftpDo?fileName=线损.docx&selectDay=2021-10-16
     * @param urlAddress
    ...
    private String encodeUrlParam(String urlAddress) {
        ...
        try {
            ...
            if (urlAddress.contains("?")) {
                ...
                //判断是否存在fileName
                Map<String, String> stringStringMap =fileUtils.urlSplit(urlAddress);
                if(stringStringMap.containsKey("fileName")) {
                    String fileName= stringStringMap.get("fileName");
                    //对中文进行编码fileName
                    String encodeLastPath = URLEncoder.encode(fileName, "UTF-8");
                    stringStringMap.put("fileName",encodeLastPath);
                    param = fileUtils.createLinkStringByGet(stringStringMap);
                }else{
